...
use bunting_nbc_market_engine::{CancelOutcome, MAX_OPEN_ORDERS, MatchError, NbcOrderBook};
...
fn submit(
    ...
    price: i64,
    quantity: i64,
) -> Result<bunting_nbc_market_engine::MatchOutcome, MatchError> {
    book.submit_limit(
        order_id,
        participant_id,
        side,
...
fn price_time_partial_fills_match_bytecode_fixture() {
    ...
    assert_eq!(
        fills
            .iter()
            .map(bunting_nbc_market_engine::Fill::order_id)
            .collect::<Vec<_>>(),
        expected
            .cases
    ...
    assert_eq!(
        fills
            .iter()
            .map(bunting_nbc_market_engine::Fill::is_maker)
            .collect::<Vec<_>>(),
