...
pub const PUBLIC_EVENT_LIMIT: usize = 32;
pub const PRIVATE_EVENT_LIMIT: usize = 64;
...
fn is_public(event: &EventEnvelope, instrument_id: InstrumentId) -> bool {
    match &event.payload {
        ...
        | EventPayload::KillSwitchActivated => false,
    }
}
...
pub fn plan(events: Vec<EventEnvelope>, current: EventSequence, class: StreamClass) -> Plan {
    ...
    if filtered.len() > limit {
        // Public projections may recover by snapshot (coalescing all intermediate levels).
        // Private facts never silently disappear; the typed reset forces origin recovery.
        return Plan::Reset {
            cursor: current,
...
fn frame(event: Option<&str>, id: Option<EventSequence>, data: &Value) -> Vec<u8> {
    ...
    output.push_str(&serde_json::to_string(data).unwrap_or_else(|_| "null".to_string()));
    output.push_str("\n\n");
    output.into_bytes()
}
...
pub fn encode<T: Serialize>(
    ...
    match plan {
        Plan::Tail(events) => frames.extend(events.into_iter().map(|event| {
            frame(None, Some(event.sequence), &json!({"result":{"data":{"type":"data","data":{"kind":"committed.event","event":event}}}}))
        })),
        Plan::Reset { cursor, reason } => {
            frames.push(frame(None, Some(cursor), &json!({"result":{"data":{"type":"data","data":{"kind":"stream.reset","afterSequence":cursor.to_string(),"reason":reason}}}})));
            if let Some(snapshot) = snapshot {
                frames.push(frame(None, Some(cursor), &json!({"result":{"data":{"type":"data","data":{"kind":"market.snapshot","snapshot":snapshot}}}})));
            }
        }
    }
    frames.push(frame(Some("return"), Some(cursor), &json!({})));
    frames
}
...
mod tests {
    ...
    fn response_is_bounded_and_closes_with_recovery_cursor() {
        ...
        assert!(String::from_utf8_lossy(&frames[1]).contains("event: return\nid: 9"));
    }
}
