...
Architecture note: this document records the completed mechanical reorganization. ADR 0018 supersedes its later selectable-engine target, ADR 0019 supersedes its standalone production orderbook-adapter target, and ADR 0022 supersedes its Worker-first deployment target. Active competition-platform work follows [`plans/competition-platform-reorganization.md`](plans/competition-platform-reorganization.md).
...
| generated Worker output | `out/bunting-worker/<version>/` | Generate, ignore and upload through release tooling |
...
1. run the existing Worker release build from `apps/bunting-worker`;
2. collect the generated JavaScript shim, Wasm module and required metadata;
3. write `out/bunting-worker/<version>/`;
...
- Worker build and D1 migration discovery work from `apps/bunting-worker`;
